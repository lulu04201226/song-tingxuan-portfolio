...
const caseDetails: Record<ProjectCaseStudy["slug"], {
 ...
 "user-behavior": {
  ...
  conclusions:[
   "全站整体转化率仅 2.40%；商品页→支付页转化率 13.83%，产生 42,756 条绝对流失量；该环节每提升 1pct，理论新增确认订单 174 单，是最高优先级杠杆。",
   "新用户最终转化率 1.47%，老用户 2.83%；转化差距集中在购买后半链路，支付页→确认页差距最大，首购流程需要独立设计。",
   "Direct 渠道原始转化率 3.03%，经用户类型 × 设备结构计算标准化转化率后为 2.41%；表面优势主要来自老用户占比更高，不能直接归因渠道流量质量。",
  ],
...
export function CaseStudy({project,charts}:{project:ProjectCaseStudy,charts:React.ReactNode}){
 ...
 <section id="overview" className="content-section reveal-section"><SectionTitle kicker="01 / OVERVIEW" title="用指标快速理解项目"/><div className="case-brief"><article><span>业务目标</span><p><HighlightNumbers>{detail.objective}</HighlightNumbers></p></article><article><span>我的角色</span><p>{detail.role}</p></article><article><span>核心分析动作</span><p><HighlightNumbers>{detail.analysis}</HighlightNumbers></p></article><article><span>最终交付</span><p><HighlightNumbers>{detail.delivery}</HighlightNumbers></p></article></div><div className="metric-grid">{displayMetrics.map(m=><div className="metric-card" key={m.label}><strong>{m.value}</strong><span>{m.label}</span>{m.note&&<small>{m.note}</small>}</div>)}</div></section>
 <section id="evidence" className="content-section reveal-section"><SectionTitle kicker="02 / EVIDENCE" title="关键数据证据" copy="图表与结论一一对应：先看数字，再看它对业务决策意味着什么。"/><div className="charts-grid">{charts}</div><div className="evidence-enrichment"><figure className="evidence-figure"><img src={project.reportEvidence.image} alt={project.reportEvidence.alt} loading="lazy"/><figcaption>{project.reportEvidence.alt}</figcaption></figure><div className="evidence-context"><h3>{project.reportEvidence.title}</h3><p><HighlightNumbers>{project.reportEvidence.takeaway}</HighlightNumbers></p><ul>{project.reportEvidence.points.map(point=><li key={point}><HighlightNumbers>{point}</HighlightNumbers></li>)}</ul></div></div><div className="insight-list">{detail.conclusions.map((x,i)=><article key={x}><span>0{i+1}</span><p><HighlightNumbers>{x}</HighlightNumbers></p></article>)}</div></section>
 <section id="method" className="content-section split-section reveal-section"><SectionTitle kicker="03 / METHOD" title="分析方法" copy="点击卡片，查看方法如何服务于这个项目的具体决策。"/><div className="method-list">{project.methods.map((x,i)=><details className="method-card" key={x}><summary><span>{String(i+1).padStart(2,"0")}</span><strong>{x}</strong><i aria-hidden>＋</i></summary><div className="method-page"><p><HighlightNumbers>{methodApplications[project.slug][i]}</HighlightNumbers></p></div></details>)}</div></section>
 <section id="action" className="content-section reveal-section density-light"><SectionTitle kicker="04 / ACTION" title="从洞察到行动" copy="按优先级安排资源，让结论进入可验证的落地路径。"/><div className="action-grid">{detail.actions.map(action=><article key={action.priority}><span>{action.priority}</span><h3>{action.title}</h3><p><HighlightNumbers>{action.copy}</HighlightNumbers></p>{action.meta&&<small><HighlightNumbers>{action.meta}</HighlightNumbers></small>}</article>)}</div></section>
 <section id="limits" className="content-section limit-section reveal-section density-light"><SectionTitle kicker="05 / BOUNDARY" title="分析边界"/><ul>{detail.limitations.map(x=><li key={x}><HighlightNumbers>{x}</HighlightNumbers></li>)}</ul></section>
