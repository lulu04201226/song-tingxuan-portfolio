...
export type ProjectCaseStudy = {
  slug: "ebay-operations" | "ab-test" | "ai-product" | "user-behavior";
...
export const projects: ProjectCaseStudy[] = [
  { slug:"ebay-operations", eyebrow:"跨境电商与数据运营", title:"eBay 跨境电商数据运营", question:"如何从 0 到 1 建立可靠的商品数据与运营闭环？", summary:"围绕 eBay 店铺冷启动，治理商品元数据、引入 LLM 提效，并持续监控商品供给与经营指标。", decision:"先统一商品信息规范与 AI 输出校验，再以 SKU 级指标复盘推动商品结构和投放策略迭代。", metrics:[{value:"0→1",label:"店铺冷启动"},{value:"SKU",label:"商品分析单元"},{value:"LLM",label:"内容处理提效"},{value:"全链路",label:"数据质量管控"}], methods:["商品元数据标准化","LLM 辅助处理与人工校验","SKU 级供给效率诊断","经营指标复盘与策略迭代"], insights:["商品标题、类目和多语种描述的统一，是稳定刊登与后续分析的基础。","AI 适合批量完成英文润色、标签提取与信息校对，但必须保留规则与人工校验环节。","库存、曝光与滞销指标需要落到 SKU 粒度，才能识别低效供给并支持商品结构调整。"], actions:["建立标题、类目、多语种描述与刊登规则的统一标准。","将 LLM 嵌入数据预处理流程，并定义输入、输出与质量校验逻辑。","持续复盘库存、定价、曝光、滞销等指标，形成商品上下架与结构优化建议。"], limitations:["项目为个人合作创业场景，部分经营数据涉及业务隐私，网站仅展示方法与脱敏结论。","业务周期集中在店铺冷启动阶段，长期复购与规模化经营效果仍需持续观察。","AI 提效效果受品类、语言与平台规则变化影响，需要持续维护校验规则。"], github:"https://github.com/lulu04201226", feishu:"#feishu-preview", reportEvidence:{label:"PROJECT CUT / 01",title:"先把商品数据做对，再让 AI 放大处理效率",image:"/visuals/data-operations-internship.webp",alt:"跨境电商商品数据治理与运营分析工作场景",takeaway:"项目把商品信息规范、AI 辅助处理和 SKU 级经营监控串成闭环，避免只追求上品速度而忽略数据质量。",points:["统一标题、类目与多语种描述，为稳定刊登和指标分析提供可靠底座。","通过 LLM 批量润色、提取标签和标准化校对，再由规则与人工机制过滤异常输出。"],note:"基于个人合作创业项目整理 · 经营信息已脱敏"} },
...
export const projectCardDetails: Record<ProjectCaseStudy["slug"], {
  ...
  "ebay-operations": {
    question: "如何从 0 到 1 建立可靠的商品数据与运营闭环？",
    goal: "支撑 eBay 店铺冷启动，提升商品信息质量与处理效率，并从供给侧优化商品结构。",
    role: "个人合作创业项目｜商品数据治理、AI 提效、指标监控",
    methods: ["商品元数据标准化", "LLM 辅助处理 + 质量校验", "SKU 级指标复盘"],
    conclusion: "建立商品信息规范与 AI 输出校验机制，形成从数据预处理、商品刊登到低效 SKU 识别的运营闭环。",
    interpretation: "商品数据质量既影响刊登稳定性，也决定曝光、转化分析与后续模型迭代是否可靠。",
    deliverable: "商品信息规范、AI 提效流程、SKU 监控口径及商品结构优化建议。",
  },
  "ab-test": {
